...
const quoteFont =
  "font-[family-name:var(--font-bebas)] text-2xl leading-snug tracking-wide sm:text-4xl lg:text-5xl";

export function FounderQuote() {
  const [emphasis, ...rest] = founder.quote.addendum.body;
  ...
  return (
    <div className="relative mx-auto max-w-3xl overflow-hidden rounded-2xl border border-border bg-surface/50 px-6 py-12 text-center sm:px-10 sm:py-16">
      ...
      <blockquote className={`relative mt-6 ${quoteFont}`}>
        &ldquo;{founder.quote.text}&rdquo;
      </blockquote>
      <div className="relative mx-auto mt-10 max-w-2xl space-y-4 text-left text-[15px] leading-[1.75] text-muted sm:text-base sm:leading-relaxed">
        <p className={`text-center text-gold ${quoteFont}`}>{emphasis}</p>
        {rest.map((paragraph) => (
